---
description: Start one or more tk tickets and implement them (unified single/multi-ID)
agent: os-tk-worker
subtask: true
background: true
---
...
- Check `unsafe.allowParallel`:
  - If `false`: STOP with message:
    ...
    Enable it in .os-tk/config.json (or config.json): unsafe.allowParallel = true
